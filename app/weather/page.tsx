...
export default function WeatherPage() {
  ...
  // Use coordinates for the center of Seychelles
  const seychellesCoordinates = [-4.6796, 55.4920]; // Centered on Seychelles islands
  ...
  useEffect(() => {
    // Set initial URL with zoom level 10 to show a closer view of Seychelles with visible islands
    const newUrl = `https://embed.windy.com/embed2.html?lat=${seychellesCoordinates[0]}&lon=${seychellesCoordinates[1]}&zoom=10&level=surface&overlay=wind&menu=&message=true&marker=true&calendar=now&pressure=&type=map&location=coordinates&detail=true&metricWind=kt&metricTemp=%C2%B0C&radarRange=-1`;
            ...
            <div className="card bg-white p-3 shadow-sm mb-4" style={{ maxWidth: '100%' }}>
              <div className="d-flex justify-content-between align-items-center mb-3">
                <h5 className="mb-0">
                  ...
                  Currently viewing: <span className="text-primary">Seychelles</span>
                </h5>
                <div>
                  <button
                  ...
                  <span className="badge bg-light text-dark">
                    ...
                    Coordinates: {seychellesCoordinates[0].toFixed(4)}, {seychellesCoordinates[1].toFixed(4)}
                  </span>
                </div>
              </div>
                ...
                {isLoading && (
                  <div className="position-absolute w-100 h-100 d-flex justify-content-center align-items-center bg-light bg-opacity-75" style={{ zIndex: 5 }}>
                    <div className="text-center">
                      ...
                      <p className="mb-0">Loading weather map for Seychelles...</p>
                    </div>
                  </div>
                )}
                {iframeUrl ? (
                  <iframe
                    key={`windy-map-seychelles-${Date.now()}`}
                    className="embed-responsive-item"
                    src={iframeUrl}
                    width="100%"
                    height="950px"
                    style={{ border: 'none' }}
                    title="Windy.com - Seychelles Weather Map"
                  ...
                  <div className="d-flex justify-content-center align-items-center bg-light" style={{ height: '950px' }}>
                    <div className="text-center">
                      ...
                      <p className="text-muted">Please wait while we load the weather map for Seychelles</p>
                    </div>
                  </div>
                )}
              </div>
